// Script to add sample data for testing reports
...
async function addSampleData() {
  try {
    console.log('Adding sample data for reports testing...')
    ...
    if (!users?.length || !cafeterias?.length || !menuItems?.length) {
      console.log('No existing users, cafeterias, or menu items found. Please create some first.')
      return
    }

    const students = users.filter(u => u.role === 'student')
    if (!students.length) {
      console.log('No student users found. Please create some student accounts first.')
      return
    }
    ...
    console.log(`- Total Commissions: ${totalCommissions.toFixed(2)} EGP`)
    console.log(`- Total Service Fees: ${totalServiceFees.toFixed(2)} EGP`)
    console.log(`- Platform Revenue: ${(totalCommissions + totalServiceFees).toFixed(2)} EGP`)
