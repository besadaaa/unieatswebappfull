...
export default function CafeteriaRatingsPage() {
  ...
  // Handle search - now works in real-time
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    // Search is handled by the filteredCafeterias computed value
    if (searchQuery.trim()) {
      toast({
        title: "Search Applied",
        description: `Filtering results for: "${searchQuery}"`,
      })
    }
  }
          ...
          <form onSubmit={handleSearch} className="relative w-64">
            ...
            <Select defaultValue={sortBy} onValueChange={setSortBy}>
              ...
              <SelectContent className="glass-effect border-white/20">
                ...
                <SelectItem value="totalRatings" className="hover:bg-emerald-500/20">Most Reviews</SelectItem>
                <SelectItem value="location" className="hover:bg-purple-500/20">Location (A-Z)</SelectItem>
              </SelectContent>
            </Select>
          ...
          <TabsContent value="overview" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {cafeteriaRatings.map((cafe) => (
                <Card key={cafe.id} className="modern-card glass-effect hover-lift animate-slide-in-up">
                  <CardHeader className="pb-2">
                    <CardTitle className="gradient-text">{cafe.name}</CardTitle>
              ...
              <CardContent>
                <div className="space-y-4">
                  {cafeteriaRatings
                    ...
                    .map((review) => (
                      <div key={review.id} className="glass-effect border-white/10 p-4 rounded-lg hover-lift transition-all duration-300">
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="font-medium text-white">{review.user}</div>
                            <div className="text-sm text-gray-300">{review.cafeteria}</div>
                            {review.menuItem && (
                            ...
                            {review.type && (
                              <div className="text-xs text-blue-400 mt-1">
                                {review.type === 'cafeteria' ? '🏪 Cafeteria Review' : '🍽️ Menu Item Review'}
                              </div>
                            )}
                          </div>
                          <div className="flex items-center">
                            <Rating initialRating={review.rating} itemId={review.id} readOnly={true} size="sm" />
                            <span className="ml-1 text-amber-400 font-medium">{review.rating}</span>
                          </div>
                        </div>
                        <p className="text-sm mt-2 text-gray-200">{review.comment}</p>
